...
//model
__constant__ double d_Dp, d_lambda, d_mean;
double h_lambda, h_mean;
...
__global__ void init_poisson(double *d_dx, int *d_pcd, curandState *d_states)
...
__device__ double adapted_jump_poisson(int &npcd, int pcd, double l_lambda, double l_Dp, double l_dt, curandState *l_state)
{
...
__device__ void predcorr(double &corrl_x, double l_x, int &npcd, int pcd, curandState *l_state, \
                         double l_Dp, double l_lambda, double l_dt)
...
*/
{
    ...
    corrl_x = l_x + 0.5*(l_xt + l_xtt)*l_dt + adapted_jump_poisson(npcd, pcd, l_lambda, l_Dp, l_dt, l_state);
}
...
__global__ void run_moments(double *d_x, double *d_dx, int *d_pcd, curandState *d_states)
